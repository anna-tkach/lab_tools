...
- `.instructions/` inside a repo is explicitly denied (`deny file-read* file-write*` on the whole subpath, not just one file) — it holds human-only docs (e.g. how to attach the SSH remote) and must stay invisible to the sandboxed agent. The instructions-folder path is computed, never hardcoded, via `compute_instructions_folder_path_by_git_instruction_file_path` / its inverse in `keys/ssh/generate/repo-remote-connection/utils_paths_computing.sh` — only `repo.sh` (top-level) knows the literal folder name (`REPO_INSTRUCTIONS_FOLDER_NAME` in the root `constants.sh`).
